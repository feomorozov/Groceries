...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try { const { supabase } = await requireApiMember(); const receipt = await getReceipt(supabase, (await params).id); return receipt ? NextResponse.json(receipt) : NextResponse.json({ error: "Receipt not found." }, { status: 404 }); }
