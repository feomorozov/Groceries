...
const ids = ["michael", "kevin", "feo", "socket"] as RoommateId[];
...
test("single person owns the full item", () => assert.deepEqual(allocateReceipt(receipt("a","michael",[[1200,["feo"]]])).portions, {michael:0,kevin:0,feo:1200,socket:0}));
...
test("tax and adjustment are allocated by positive item value", () => { const r=receipt("a","michael",[[300,["kevin"]],[700,["feo"]]],{taxCents:101,adjustmentCents:-1,totalCents:1100}); const a=allocateReceipt(r); assert.deepEqual(a.items.map(x=>x.adjustmentCents),[30,70]); assert.deepEqual(a.portions,{michael:0,kevin:330,feo:770,socket:0}); });
